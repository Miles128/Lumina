...
def rule_route(message: str) -> GateDecision | None:
    ...
    from secretary.agent.grounding import is_memory_write_request, is_personal_memory_question

    if is_memory_write_request(text):
        return GateDecision(action=GateAction.CONTINUE, reason="memory write")
    if is_personal_memory_question(text):
