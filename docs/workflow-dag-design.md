...
**日期：** 2026-07-09 · **拍板：** 2026-07-22  
**状态：** 设计已拍板 · 后端 + Drawflow 画布编辑器已落地 · 运行高亮已支持  
**PRD：** F26（见 `docs/PRD.md` §11）

### 拍板记录（2026-07-22）

| # | 决策 | 结论 |
|---|------|------|
| A | 入口 | 独立「工作流」tab；分支地图留对话内 |
| B | v1 节点 | Skill + Agent + Branch(`expr` only)；Branch(`agent`) → v1.1 |
| C | 暂停 | v1 一口气跑完；写工具确认 → v2 路 1 |
| D | Shell | 先加工作流入口，左侧大改导航可后置 |
| E | 切片顺序 | ① Store + Scheduler + `/api/workflows` → ② 画布 UI |

> 产品定位：Lumina 是本地通用 Agent 生产力工具。对话地图已交付；本设计描述的 Skill 编排是**规划中**差异点，勿在 README/UI 写成已上线。
