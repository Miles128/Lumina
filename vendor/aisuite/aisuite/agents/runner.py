...
from ..utils.tools import ApprovalRequiredError
    ...
    async def _run_impl(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            agent_step.data["status"] = "requires_input"
            agent_step.data["pending_tool"] = {
                "name": exc.tool_name,
                "arguments": copy.deepcopy(exc.arguments),
                "tool_call_id": exc.tool_call_id,
                "reason": exc.reason,
                "metadata": copy.deepcopy(exc.metadata),
            }
            paused_messages = (
                copy.deepcopy(exc.pending_messages)
                if getattr(exc, "pending_messages", None)
                else copy.deepcopy(messages)
            )
            pending_meta = {
                **effective_metadata,
                "pending_approval": agent_step.data["pending_tool"],
            }
            result = RunResult(
                final_output=None,
                ...
                new_items=[],
                raw_responses=[exc.raw_response] if exc.raw_response is not None else [],
                run_name=effective_run_name,
                trace_id=active_trace_id,
                parent_run_id=effective_parent_run_id,
                group_id=effective_group_id,
            ...
            Runner._emit_trace_event(
                ...
                tags=effective_tags,
                metadata=pending_meta,
                data=agent_step.data["pending_tool"],
            )
            if state_store is not None and thread_id is not None:
                state_store.save_state(thread_id, result.to_state())
            return result
