...
class ApprovalRequiredError(Exception):
    """Lumina fork: human approval required — pause the run (do not deny into model)."""
    ...
    def __init__(
        ...
        *,
        ...
        self.reason = reason or "needs_human_approval"
        self.metadata = metadata or {}
        self.pending_messages: list = []
        self.raw_response = None
        super().__init__(
            f"Approval required for tool {tool_name!r}: {self.reason}"
        )
...
class Tools:
    ...
    def _prepare_tool_call(self, tool_call, tool_policy, tool_policy_context) -> dict:
        ...
        if decision is not None and not decision.allowed:
            # Lumina fork: pause for human approval instead of denying into the model.
            if (decision.metadata or {}).get("pause_for_approval"):
                raise ApprovalRequiredError(
                    tool_name=tool_name,
                    arguments=validated_args_dict,
                    tool_call_id=tool_call_id,
                    reason=decision.reason,
